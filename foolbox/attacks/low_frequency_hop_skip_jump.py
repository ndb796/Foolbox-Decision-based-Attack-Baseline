...
def sample_gaussian_torch(image_size, dct_ratio=0.125):
    x = torch.zeros(image_size)
    fill_size = int(image_size[-1] * dct_ratio)
    x[:, :, :, :fill_size, :fill_size] = torch.randn(x.size(0), x.size(1), x.size(2), fill_size, fill_size)
    if dct_ratio < 1.0:
        x = torch.from_numpy(idct(idct(x.numpy(), axis=4, norm='ortho'), axis=3, norm='ortho'))
    return x
